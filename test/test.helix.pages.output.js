...
describe('Basic test of the Helix Pages output', () => {
  it('contains our content', (done) => {
    request(testURL, async (err, res, body) => {
      ...
      // page title
      assert.equal(expectedTitle, $('h1:first').text());
      ...
      // images
      const imageSrcSet = $('img').attr('src');
      expectedImgSrc.forEach((src) => {
        assert(
          imageSrcSet.indexOf(src) > 0,
          `Expecting '${src})' to be found as an a:href attribute`,
        );
      });

      // signal mocha that we're done
      done();
    });
